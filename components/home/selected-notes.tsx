...
export function SelectedNotes({ items }: { items: ContentItem[] }) {
  return (
    <section id="notes" aria-labelledby="notes-heading" className="border-b border-border">
      <div className="mx-auto max-w-6xl px-6 py-24 md:px-8 md:py-32">
        <header className="flex items-end justify-between gap-8 border-b border-border pb-6">
          <h2 id="notes-heading" className="text-3xl font-medium tracking-tight md:text-5xl">
            文章与笔记
          </h2>
          <Link
            href="/knowledge"
        ...
        {items.length > 0 ? (
          ...
          <p className="border-b border-border py-12 text-muted-foreground">更多文章正在整理中</p>
        )}
      </div>
    </section>
  )
