...
describe("SelectedProjects", () => {
  it("renders secure external project links and the internal projects index", () => {
    ...
    const projectLink = screen.getByRole("link", { name: /Agent Workbench/ })
    expect(projectLink).toHaveAttribute("href", linkedProject.href)
    expect(projectLink).toHaveAttribute("target", "_blank")
    expect(projectLink).toHaveAttribute("rel", "noopener noreferrer")
    expect(projectLink).toHaveClass("inline-flex", "min-h-11", "items-center")
...
describe("SelectedNotes", () => {
  it("presents notes without recency claims and uses secure external links", () => {
    ...
    const noteLink = screen.getByRole("link", { name: /可靠 Agent 工作流的几个切面/ })
    expect(noteLink).toHaveAttribute("target", "_blank")
    expect(noteLink).toHaveAttribute("rel", "noopener noreferrer")
    expect(noteLink).toHaveClass("inline-flex", "min-h-11", "items-center")
    const firstRow = screen.getByRole("listitem")
