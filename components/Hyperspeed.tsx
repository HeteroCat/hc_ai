...
interface HyperspeedProps {
  /**
   * 预设名称, 可从 `hyperspeedPresets` 中选择, 用于快速切换配置
   * @default 使用默认的 `defaultOptions`
   */
...
const Hyperspeed: FC<HyperspeedProps> = ({ preset, effectOptions = {} }) => {
  // 先取默认配置
  let baseOptions: HyperspeedOptions = { ...defaultOptions };
  // 如果提供了预设, 合并对应的预设配置
  if (preset && typeof hyperspeedPresets[preset] !== 'undefined') {
    baseOptions = { ...baseOptions, ...hyperspeedPresets[preset] } as HyperspeedOptions;
  }
  // 最后合并用户自定义的 effectOptions, 覆盖前面的值
